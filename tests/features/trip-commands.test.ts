...
const makeRepository = (): TripCommandsRepository & {
  ...
  const repository: TripCommandsRepository & { trip: any; checkIns: any[]; alertEvents: any[]; calls: string[] } = {
    ...
    replacePendingAlertSchedule: async ({ plannedFinishAt }) => {
      repository.calls.push('replace-alerts');
      repository.alertEvents = repository.alertEvents.map((event) => event.status === 'pending' ? { ...event, status: 'cancelled' } : event);
    ...
    cancelPendingAlerts: async () => { repository.calls.push('cancel-alerts'); repository.alertEvents = repository.alertEvents.map((event) => event.status === 'pending' ? { ...event, status: 'cancelled' } : event); },
  };
  return repository;
};
...
describe('trip lifecycle commands', () => {
  it('starts a draft trip only with a fresh GPS fix', async () => {
    ...
    await startTrip({ tripId: 'trip-1', userId: 'leader-1', location: freshGps, idempotencyKey: 'start-1', now }, repository);
  ...
  it('lets a member record a text-only check-in with explicit unavailable location', async () => {
    ...
    await recordCheckIn({ tripId: 'trip-1', userId: 'member-1', message: '平安，無定位訊號', idempotencyKey: 'check-in-1', now }, repository);
  ...
  it('allows only leader or deputy to extend and atomically replaces unsent alerts', async () => {
    ...
    await extendTrip({ tripId: 'trip-1', userId: 'deputy-1', plannedFinishAt, idempotencyKey: 'extend-1', now }, repository);
