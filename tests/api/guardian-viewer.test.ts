...
describe('guardian viewer route', () => {
  ...
  it('returns the route, team, last check-in location accuracy, and report for the bound LINE guardian', async () => {
    ...
    vi.mocked(loadGuardianViewer).mockResolvedValue({
      route: '玉山主峰', team: ['阿山'], lastCheckIn: { at: '2026-07-12T04:20:00.000Z', location: { latitude: 23.47, longitude: 120.95, accuracyMeters: 12 } }, report: '119 摘要',
    });
    ...
    await expect(response.json()).resolves.toMatchObject({ route: '玉山主峰', team: ['阿山'], lastCheckIn: { location: { accuracyMeters: 12 } }, report: '119 摘要' });
