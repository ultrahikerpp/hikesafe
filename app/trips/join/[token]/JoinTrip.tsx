...
export function JoinTrip({ token }: { token: string }) {
  ...
  return <main><LiffBootstrap onReady={() => setReady(true)} /><h1>加入 BeSafe 小隊</h1><p>請以 LINE 登入後確認加入；加入後由隊長指定副領隊。</p><button disabled={!ready} onClick={() => void join()}>加入行程</button>{notice && <p role="alert">{notice}</p>}</main>;
}
