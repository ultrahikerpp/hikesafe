...
export const GET = async (request: Request, { params }: { params: Promise<{ tripId: string }> }) => {
  ...
  if (new URL(request.url).searchParams.get('format') === 'text') {
    return new Response(report.text, { headers: { 'content-type': 'text/plain; charset=utf-8', 'Cache-Control': 'no-store', 'Referrer-Policy': 'no-referrer' } });
  }
  return NextResponse.json(report, { headers: { 'Cache-Control': 'no-store', 'Referrer-Policy': 'no-referrer' } });
};
