...
import { startTrip } from '@/src/features/trips/commands';
...
const requestSchema = z.object({ location: locationSchema, idempotencyKey: z.string().min(1) }).strict();
...
export const handleStart = async (request: Request, { params }: { params: Promise<{ tripId: string }> }) => {
  ...
  try {
    const result = await startTrip({ ...(await params), ...parsed.data, userId: session.userId, now: new Date() });
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to start trip' }, { status: 422 });
  }
};
