...
import { recordCheckIn } from '@/src/features/trips/commands';
...
export const handleCheckIn = async (request: Request, { params }: { params: Promise<{ tripId: string }> }) => {
  ...
  if (!parsed.success || (!parsed.data.message && !parsed.data.location)) return NextResponse.json({ error: 'Invalid request' }, { status: 400 });
  try {
    const result = await recordCheckIn({ ...(await params), ...parsed.data, location: parsed.data.location, userId: session.userId, now: new Date() });
    return NextResponse.json(result, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to record check-in' }, { status: 422 });
  }
};

export const POST = handleCheckIn;
