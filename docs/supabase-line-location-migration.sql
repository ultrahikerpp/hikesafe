-- For the already-bootstrapped Supabase database, run this directly instead of replaying Drizzle migrations.
...
ALTER TABLE "public"."check_ins" ADD CONSTRAINT "check_ins_location_consistency" CHECK (
  (
    ...
    AND (("location_source" IN ('gps', 'network') AND "accuracy_meters" IS NOT NULL)
      OR ("location_source"::text = 'line' AND "accuracy_meters" IS NULL))
  )
  OR (
