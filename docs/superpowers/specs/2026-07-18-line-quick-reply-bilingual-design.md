...
「傳送位置」使用 LINE 原生位置 action。收到位置 message event 後，將位置轉為 `locationSource = 'line'` 的 check-in，保存經緯度與收到時間，`accuracyMeters` 留空表示 LINE 未提供精度；位置回報本身不推播給留守人。此資料可供留守查看與後續通報摘要使用，但不得作為開始登山所需的新鮮 GPS。
...
Migration 將 `location_source` enum 增加 `line`，並調整 `check_ins_location_consistency`：`gps` 與 `network` 位置仍必須有精度；`line` 位置必須有經緯度與時間，但精度可為 NULL。既有資料的約束與 GPS 行為不變。

`LocationFix` 維持 GPS／network 的精度要求；另新增只供 check-in 的 LINE location 型別，避免把缺少精度的 LINE 位置傳入開始登山流程。
...
- 本次新增 LINE 對話回報、LINE 位置保存、Quick Reply 與全功能文字雙語化。
- 新增一份 migration；不得修改既有 migration 檔案。
- 不新增 Rich Menu。
- 不將一般聊天文字自動判斷為回報。
- 不將所有路線資料或使用者輸入自動翻譯。
...
- 留守查看與通報摘要會顯示 LINE 位置，但明確標示精度未知，不顯示假的 GPS 精度。
...
- migration 在乾淨資料庫與既有資料庫都可套用，且執行 `npm test` 與 `npm run build`。
