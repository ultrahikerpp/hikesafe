...
每筆路線必須完整提供名稱、slug、區域、既有類型、起點座標、距離、時間、難度、檢查點、許可資訊、來源組織、來源 URL、來源版本與審核日期。`elevationGainM` 僅在官方明確發布累積爬升時填入；官方只發布高度落差時，以 `elevationDifferenceM` 儲存該原始語意。官方未載明撤離路線或撤離點時，`evacuationPoints` 為空陣列，產品與緊急報告須顯示「官方資料未載明」。
