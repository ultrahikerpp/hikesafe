...
`sources.json` 登錄每個來源組織與 URL；`catalog.json` 保留 `sourceOrganization` 與 `sourceUrl` 作為主要來源，並以 `sourceReferences` 記錄各欄位所依據的官方來源。每個參照都必須對應來源登錄，並標示其支持的欄位。官方資料若缺少必要欄位，改找另一個官方來源；若仍未發布，保留未提供狀態，不自行補值。不得將「高度落差」轉填為累積爬升、由 KML 計算累積爬升，或以地理距離將 GIS 救難點配對到路線。
