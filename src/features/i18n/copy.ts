...
export const copy = {
  ...
  routeSourceMissingDuration: (organization: string, version: string, reviewedAt: string) => bilingual(
    `時間資料未載明，請自行預留下山時間；來源：${organization}；版本 ${version}；覆核 ${reviewedAt}`,
    `Duration not published; plan your own descent time. Source: ${organization}; version ${version}; reviewed ${reviewedAt}`,
  ),
  communitySourceWarning: bilingual(
    '路線資料含社群來源，行前請自行確認現況',
    'This route includes community-sourced data; verify current conditions before departure',
  ),
